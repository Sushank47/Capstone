...
import { MessageSquare, Phone, Video, ShieldCheck, Send, Mic, MicOff, VideoOff, PhoneOff, X, User } from 'lucide-react';
...
export const DoctorPatientChat: React.FC<Props> = ({ consultation, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
        ...
        <div className="flex items-center justify-between pb-4 border-b dark:border-slate-800 border-slate-200 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 rounded-xl bg-teal-500/10 hover:bg-teal-500/20 text-teal-700 dark:text-teal-300 border border-teal-500/30 font-bold text-xs flex items-center gap-1.5 transition-all shadow-sm"
            ...
            <button
              ...
              className="p-2 rounded-xl bg-teal-500 hover:bg-teal-400 text-slate-950 font-bold text-xs flex items-center gap-1.5 transition-all shadow-sm"
      ...
      {/* Audio / Video Telehealth Call Simulation Modal Overlay */}
      {activeCallType && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-lg animate-in fade-in">
          <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-700 rounded-3xl p-6 text-white space-y-6 shadow-2xl text-center">
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <span className="text-xs text-emerald-400 font-mono font-bold animate-pulse">● LIVE CONNECTED</span>
            </div>

            {/* Video Viewport / Audio Avatar */}
            <div className="w-full h-64 rounded-2xl bg-slate-950 border border-slate-800 flex flex-col items-center justify-center relative overflow-hidden shadow-inner">
              {activeCallType === 'VIDEO' && !isVideoOff ? (
                <div className="absolute inset-0 bg-gradient-to-tr from-slate-950 via-teal-950/30 to-slate-950 flex flex-col items-center justify-center space-y-3">
                  <div className="w-20 h-20 rounded-full bg-teal-500/20 text-teal-300 flex items-center justify-center font-bold text-2xl border-2 border-teal-500/50 shadow-lg shadow-teal-500/20 animate-pulse">
                    {consultation.doctor_name.charAt(4) || 'D'}
                  </div>
                  <p className="text-sm font-bold text-white">{consultation.doctor_name}</p>
                  <p className="text-xs text-slate-400 font-medium">HD Video Stream Active (720p 60fps)</p>
                </div>
              ) : (
                <div className="space-y-3">
                  <div className="w-20 h-20 rounded-full bg-slate-800 text-slate-400 flex items-center justify-center mx-auto border border-slate-700">
                    <User className="w-10 h-10" />
                  </div>
                  <p className="text-xs text-slate-400 font-medium">
                    {isVideoOff ? 'Video Feed Turned Off' : 'Encrypted Audio Stream Active'}
                  </p>
                </div>
              )}
            </div>
